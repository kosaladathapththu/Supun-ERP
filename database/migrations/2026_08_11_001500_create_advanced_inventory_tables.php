...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $t->foreignId('from_location_id')->constrained('stock_locations')->restrictOnDelete();
        $t->foreignId('to_location_id')->constrained('stock_locations')->restrictOnDelete();
        ...
        $t->id();
        $t->foreignId('company_id')->constrained()->cascadeOnDelete();
        $t->foreignId('stock_location_id')->constrained()->restrictOnDelete();
        $t->foreignId('created_by')->constrained('users')->restrictOnDelete();
        $t->string('document_number', 30);
        $t->dateTime('adjustment_date');
        $t->string('adjustment_type', 30)->index();
        $t->string('status', 20)->default('posted');
        $t->text('reason');
        ...
        $t->foreignId('product_id')->constrained()->restrictOnDelete();
        $t->decimal('quantity_change', 18, 4);
        $t->decimal('unit_cost', 18, 4);
        $t->decimal('value_change', 18, 2);
        $t->timestamps();
        });
