...
class PhaseTwoMasterDataTest extends TestCase
{
    ...
    public function test_admin_can_validate_preview_and_confirm_master_data_csv(): void
    {
        ...
        $restockBatch = ImportBatch::latest('id')->firstOrFail();
        $this->post(route('imports.confirm', $restockBatch))->assertSessionHasNoErrors();
        ...
        $invoice = \App\Models\SupplierInvoice::where('supplier_invoice_number', 'SINV-200')->firstOrFail();
        $this->assertDatabaseHas('journal_entries', ['source_type' => \App\Models\SupplierInvoice::class, 'source_id' => $invoice->id]);
    }
    ...
    public function test_native_excel_workbook_validates_every_row(): void
    {
        ...
        foreach ($rows as $r => $values) {
            ...
            foreach ($values as $c => $value) {
                $column = chr(65 + $c);
                $sheet .= '<c r="'.$column.($r + 1).'" t="inlineStr"><is><t>'.htmlspecialchars($value, ENT_XML1).'</t></is></c>';
            }$sheet .= '</row>';
        ...
        $this->assertSame(2, $batch->valid_rows);
        $this->assertSame('001234567890', $batch->rows()->first()->data['barcode']);
    }
}
