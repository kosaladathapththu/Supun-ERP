...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div><h1 class="h3 page-title mb-1">{{ $selectedInvoice ? 'Pay '.$selectedInvoice->document_number : 'Pay a Supplier' }}</h1><p class="text-muted mb-0">Enter the payment once. The system applies it to outstanding bills automatically.</p></div>
...
<form method="POST" action="{{ route('payables.store') }}" id="supplier-payment-form">@csrf
    ...
    <div class="row g-4">
        <div class="col-xl-7">
            ...
            <div class="card"><div class="card-body p-0"><div class="p-4 pb-2"><h2 class="h5 mb-1">Outstanding bills</h2><p class="small text-muted">Payment is applied to the selected bill first, then oldest bills.</p></div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th class="ps-4">Bill</th><th>Due date</th><th class="text-end pe-4">Balance</th></tr></thead><tbody>@forelse($invoices as $invoice)<tr class="{{ $invoiceId===$invoice->id?'table-primary':'' }}"><td class="ps-4"><strong>{{ $invoice->document_number }}</strong><div class="small text-muted">{{ $invoice->supplier_invoice_number ?: 'No supplier reference' }}</div></td><td>{{ optional($invoice->due_date)->format('d M Y') ?: '—' }}</td><td class="text-end pe-4 fw-semibold">Rs. {{ number_format($invoice->balance_amount,2) }}</td></tr>@empty<tr><td colspan="3" class="text-center text-muted py-4">No outstanding bills. This payment will be saved as a supplier advance.</td></tr>@endforelse</tbody></table></div></div>
        </div>
        <div class="col-xl-5"><div class="card position-sticky" style="top:92px"><div class="card-body p-4">
            ...
            <div class="alert alert-info mt-4 mb-3 small"><i class="bi bi-info-circle me-1"></i> Partial payments are allowed. Any amount above the outstanding bills becomes a supplier advance.</div>
