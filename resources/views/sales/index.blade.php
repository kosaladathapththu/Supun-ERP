...
<form id="sales-filter-form">
    <div class="section-surface border rounded-3 overflow-hidden">
        ...
        <div class="p-3">
            <div class="small fw-semibold text-uppercase text-muted mb-2">Sales details</div>
            <div class="row g-3 mb-4">
                <div class="col-lg-4"><label class="form-label">Invoice or customer</label><input class="form-control" name="search" value="{{ request('search') }}" placeholder="Enter invoice number or customer name"></div>
                ...
                <div class="col-md-4 col-lg-3"><label class="form-label">Sale type</label><select class="form-select" name="payment_type"><option value="">Cash &amp; credit</option><option value="cash" @selected(request('payment_type')==='cash')>Cash</option><option value="credit" @selected(request('payment_type')==='credit')>Credit</option></select></div>
                <div class="col-md-4 col-lg-3"><label class="form-label">Payment status</label><select class="form-select" name="status"><option value="">All statuses</option>@foreach(['paid','partially_paid','unpaid'] as $status)<option value="{{ $status }}" @selected(request('status')===$status)>{{ Str::headline($status) }}</option>@endforeach</select></div>
            </div>
            <div class="small fw-semibold text-uppercase text-muted mb-2">Date period</div>
            <div class="row g-3 align-items-end">
                <div class="col-md-auto">
                    <label class="form-label d-block">Show sales for</label>
                    <div class="btn-group" role="group" aria-label="Date filter mode">
                        <input type="radio" class="btn-check" name="date_mode" id="date-mode-single" value="single" @checked($dateMode==='single')>
                        <label class="btn btn-outline-primary" for="date-mode-single"><i class="bi bi-calendar-day me-1"></i>Single date</label>
                        <input type="radio" class="btn-check" name="date_mode" id="date-mode-range" value="range" @checked($dateMode==='range')>
                        <label class="btn btn-outline-primary" for="date-mode-range"><i class="bi bi-calendar-range me-1"></i>Date range</label>
                    </div>
                </div>
                <div class="col-md-3" id="single-date-field"><label class="form-label">Select date</label><input class="form-control" type="date" name="date" value="{{ $singleDate }}"></div>
                <div class="col-md-3" id="from-date-field"><label class="form-label">Start date</label><input class="form-control" type="date" name="from" value="{{ $dateMode==='range' ? $from : '' }}"></div>
                <div class="col-md-3" id="to-date-field"><label class="form-label">End date</label><input class="form-control" type="date" name="to" value="{{ $dateMode==='range' ? $to : '' }}"></div>
                <div class="col-md-auto ms-md-auto"><button class="btn btn-primary px-4"><i class="bi bi-search me-1"></i>Apply filters</button></div>
            </div>
        </div>
    </div>
</form></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleDateFields = () => {
        const isSingle = document.getElementById('date-mode-single').checked;
    ...
    modes.forEach(mode => mode.addEventListener('change', toggleDateFields));
    toggleDateFields();
});
</script>
@endpush
