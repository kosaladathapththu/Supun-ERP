...
@forelse($products as $product)<tr><td class="ps-4"><strong>{{ $product->name }}</strong><div class="small text-muted">{{ $product->item_code }}</div></td><td>{{ $product->category->name }}</td><td class="fw-semibold">{{ number_format($product->current_quantity,4) }} {{ $product->unit->code }}</td><td>Rs. {{ number_format($product->average_cost,4) }}</td><td>Rs. {{ number_format($product->current_quantity*$product->average_cost,2) }}</td><td>@if($product->current_quantity<=$product->reorder_level)<span class="badge text-bg-warning">Low stock</span>@else<span class="badge text-bg-success">OK</span>@endif</td>
