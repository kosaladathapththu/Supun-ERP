...
use App\Models\{GoodsReceivedNote,PurchaseOrder,SupplierInvoice};
...
class ImportController extends Controller
{
    ...
    private function validateRow(array $d, int $companyId): array
    {
        ...
        $productId=DB::table('products')->where('company_id',$companyId)->where('item_code',(string)($d['item_code']??''))->value('id');if($productId&&is_numeric($quantity)&&(float)$quantity>0&&trim((string)($d['supplier_code']??''))==='')$e[]='supplier_code is required when replenishing an existing product';
    ...
    private function importRow(array $d, int $companyId, int $userId, int $batchId): string
    {
        ...
        $productData=['product_category_id'=>$categoryId,'brand_id'=>$brandId,'unit_id'=>$unitId,'barcode'=>($d['barcode']??'')!==''?(string)$d['barcode']:null,'name'=>$d['product_name'],'minimum_stock'=>$d['minimum_stock']?:0,'reorder_level'=>$d['reorder_level']?:0,'warranty_months'=>$d['warranty_months']?:0,'serial_tracking'=>in_array(strtolower((string)($d['serial_tracking']??'')),['yes','true','1'],true),'is_active'=>true];if(!$hadMovements)$productData['average_cost']=$d['cost_price'];$product=Product::updateOrCreate(['company_id'=>$companyId,'item_code'=>(string)$d['item_code']],$productData);
        ...
        $quantity=(string)($d['quantity']??$d['opening_quantity']??0);if(!is_numeric($quantity)||bccomp($quantity,'0',4)<=0)return'0';
