...
{
    private const HEADERS = ['supplier_code','supplier_name','supplier_phone','item_code','product_name','barcode','brand','unit','category','cost_price','retail_price','wholesale_price','minimum_stock','reorder_level','warranty_months','serial_tracking','opening_quantity'];
    ...
    {
        $lines = [self::HEADERS, ['SUP-001','Demo Supplier','0110000000','ITEM-001','Demo Product','890000000001','Demo Brand','PCS','Electronics','1000.00','1250.00','1150.00','2','5','12','yes','10']];
    ...
    public function confirm(Request $request, ImportBatch $batch)
    {
        ...
        DB::transaction(function () use ($batch, $request) {
            ...
            if(bccomp($openingValue,'0',2)>0)app(\App\Services\JournalPostingService::class)->post($batch->company_id,now()->toDateString(),ImportBatch::class,$batch->id,'IMPORT-'.$batch->uuid,'Opening inventory imported from '.$batch->original_filename,[['account_code'=>'1140','debit'=>$openingValue],['account_code'=>'3300','credit'=>$openingValue]],$request->user()->id);
    ...
    private function validateRow(array $d, int $companyId): array
    {
        ...
        if(($d['opening_quantity']??'')!==''&&(!is_numeric($d['opening_quantity'])||$d['opening_quantity']<0))$e[]='opening_quantity must be a non-negative number';
        if(is_numeric($d['opening_quantity']??null)&&(float)$d['opening_quantity']>0){$productId=DB::table('products')->where('company_id',$companyId)->where('item_code',(string)($d['item_code']??''))->value('id');if($productId&&DB::table('stock_movements')->where('product_id',$productId)->exists())$e[]='opening_quantity cannot be applied because this product already has stock movements';}
    ...
    private function importRow(array $d, int $companyId, int $userId, int $batchId): string
    {
        ...
        $quantity=(string)($d['opening_quantity']??0);if(!is_numeric($quantity)||bccomp($quantity,'0',4)<=0)return'0';
        $location=DB::table('stock_locations')->where('company_id',$companyId)->where('is_default',1)->value('id');$value=bcmul($quantity,(string)$d['cost_price'],2);$product->update(['current_quantity'=>$quantity]);DB::table('stock_movements')->insert(['company_id'=>$companyId,'product_id'=>$product->id,'stock_location_id'=>$location,'created_by'=>$userId,'movement_at'=>$now,'movement_type'=>'opening','reference_type'=>ImportBatch::class,'reference_id'=>$batchId,'reference_number'=>'IMPORT-'.$batchId,'quantity_in'=>$quantity,'quantity_out'=>0,'balance_quantity'=>$quantity,'unit_cost'=>$d['cost_price'],'stock_value'=>$value,'notes'=>'Opening stock from master-data import','created_at'=>$now,'updated_at'=>$now]);return$value;
    }
}
