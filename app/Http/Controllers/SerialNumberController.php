...
class SerialNumberController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->boolean('expiring')) {
            $query->whereBetween('warranty_expires_on', [now()->toDateString(), now()->addDays(30)->toDateString()]);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (! $serials) {
            throw ValidationException::withMessages(['serials' => 'Enter at least one serial number.']);
        }
        $existing = ProductSerialNumber::whereIn('serial_number', $serials)->pluck('serial_number')->all();
        if ($existing) {
            throw ValidationException::withMessages(['serials' => 'Already registered: '.implode(', ', array_slice($existing, 0, 10))]);
        }
        foreach ($serials as $serial) {
            ProductSerialNumber::create(['product_id' => $data['product_id'], 'stock_location_id' => $data['stock_location_id'] ?? null, 'serial_number' => $serial, 'status' => 'available']);
        }
