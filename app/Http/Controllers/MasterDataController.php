...
abstract class MasterDataController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($search = trim((string) $request->query('search'))) $query->where(fn ($q) => $q->where('code', 'like', "%{$search}%")->orWhere('name', 'like', "%{$search}%"));
    ...
    {
        $data = $this->data($request); $data['company_id'] = $request->user()->company_id;
    ...
    {
        $record = ($this->model)::findOrFail($record);
        $this->authorizeRecord($record); $record->update($this->data($request));
    ...
    protected function parents(?int $exclude = null) { return collect(); }
    protected function authorizeRecord($record): void { abort_unless($record->company_id === auth()->user()->company_id, 404); }
    protected function viewData(array $data): array { return $data + ['route'=>$this->route,'title'=>$this->title,'extraFields'=>$this->extraFields]; }
}
