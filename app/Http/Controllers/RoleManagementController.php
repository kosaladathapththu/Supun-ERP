...
class RoleManagementController extends Controller
{
 ...
 public function update(Request $r,$role){$role=Role::where('company_id',$r->user()->company_id)->findOrFail($role);if($role->slug==='main-admin')throw ValidationException::withMessages(['role'=>'The full-access CFO profile is protected.']);$data=$r->validate(['name'=>['required','string','max:100'],'description'=>['nullable','string','max:500'],'permissions'=>['nullable','array'],'permissions.*'=>['integer',Rule::exists('permissions','id')]]);$role->update(['name'=>$data['name'],'description'=>$data['description']??null]);$role->permissions()->sync($data['permissions']??[]);return redirect()->route('admin.roles.index')->with('success','Role permissions updated.');}
}
