...
class PartyRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return ['code' => ['required', 'string', 'max:50', Rule::unique($table)->where(fn ($q) => $q->where('company_id', $this->user()->company_id))->ignore($id)], 'name' => ['required', 'string', 'max:150'], 'business_name' => ['nullable', 'string', 'max:150'], 'contact_person' => ['nullable', 'string', 'max:150'], 'phone' => ['nullable', 'string', 'max:30'], 'email' => ['nullable', 'email', 'max:255'], 'address' => ['nullable', 'string', 'max:1000'], 'opening_balance' => [$customer ? 'nullable' : 'required', 'numeric', 'min:0'], 'opening_balance_date' => [$customer ? 'nullable' : 'required_if:opening_balance,>,0', 'nullable', 'date'], 'credit_enabled' => ['nullable', 'boolean'], 'default_due_term' => ['nullable', Rule::in(['7_days', '14_days', '30_days', 'end_of_month', 'custom'])], 'default_credit_days' => ['nullable', 'integer', 'between:0,365'], 'is_active' => ['nullable', 'boolean']];
    }
}
