...
class InventoryReceivingService
{
 ...
 {
  $product=Product::whereKey($product->id)->lockForUpdate()->firstOrFail();$qty=(string)$quantity;$cost=(string)$unitCost;if(bccomp($qty,'0',4)<=0||bccomp($cost,'0',4)<0)throw new RuntimeException('Received quantity must be positive and cost cannot be negative.');
  $beforeQty=(string)$product->current_quantity;$beforeCost=(string)$product->average_cost;$beforeValue=bcmul($beforeQty,$beforeCost,8);$receivedValue=bcmul($qty,$cost,8);$afterQty=bcadd($beforeQty,$qty,4);$afterCost=bccomp($afterQty,'0',4)===0?'0':bcdiv(bcadd($beforeValue,$receivedValue,8),$afterQty,4);
