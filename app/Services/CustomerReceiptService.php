        ...
        return DB::transaction(function () use ($data, $user) {
            ...
            if (! $hasManualAllocation && ! ($data['keep_unapplied'] ?? false)) {
                ...
                foreach ($openSales as $sale) {
                    ...
                    $allocations[$sale->id] = $value;
                    $remaining = bcsub($remaining, $value, 2);
                }
            }
            foreach ($allocations as $saleId => $value) {
                ...
                if (bccomp((string) $value, (string) $sale->balance_amount, 2) > 0) {
                    throw ValidationException::withMessages(['allocations' => "Allocation exceeds the balance of {$sale->document_number}."]);
                }
                $allocated = bcadd($allocated, (string) $value, 2);
                $lines[] = [$sale, (string) $value];
            }
            if (bccomp($allocated, $amount, 2) > 0) {
                throw ValidationException::withMessages(['allocations' => 'Total allocation cannot exceed the receipt amount.']);
            }
            ...
            foreach ($lines as [$sale,$value]) {
                $receipt->allocations()->create(['sale_id' => $sale->id, 'amount' => $value]);
                $paid = bcadd((string) $sale->paid_amount, $value, 2);
                $balance = bcsub((string) $sale->grand_total, $paid, 2);
                $sale->update(['paid_amount' => $paid, 'balance_amount' => $balance, 'payment_status' => bccomp($balance, '0', 2) <= 0 ? 'paid' : 'partially_paid']);
            }
            $journal = [['account_code' => $data['payment_method'] === 'cash' ? '1110' : '1120', 'debit' => $amount]];
            if (bccomp($allocated, '0', 2) > 0) {
                $journal[] = ['account_code' => '1130', 'credit' => $allocated, 'customer_id' => $data['customer_id']];
            }$advance = bcsub($amount, $allocated, 2);
            if (bccomp($advance, '0', 2) > 0) {
                $journal[] = ['account_code' => '2150', 'credit' => $advance, 'customer_id' => $data['customer_id']];
            }app(JournalPostingService::class)->post($user->company_id, $data['receipt_date'], CustomerReceipt::class, $receipt->id, $receipt->receipt_number, "Customer receipt {$receipt->receipt_number}", $journal, $user->id);
